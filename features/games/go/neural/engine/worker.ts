...
let loadedModelCacheStatus: 'hit' | 'stored' | 'unavailable' = 'unavailable';
...
async function initWasmBackend(): Promise<void> {
  try {
    setWasmPaths(publicUrl('assets/go/tfjs/'));
    ...
    const initialized = await tf.setBackend('wasm');
    if (!initialized) throw new Error('WASM backend unavailable');
    await tf.ready();
    if (tf.getBackend() !== 'wasm') throw new Error('WASM backend unavailable');
    return;
  } catch {
    // Fall through to CPU below.
  }

  const initialized = await tf.setBackend('cpu');
  if (!initialized) throw new Error('CPU backend unavailable');
  await tf.ready();
...
async function initBackend(preferredBackend: KataGoBackendPreference): Promise<void> {
  ...
  if (preferredBackend === 'webgpu') {
    try {
      const initialized = await tf.setBackend('webgpu');
      if (!initialized) throw new Error('WebGPU backend unavailable');
      await tf.ready();
      if (tf.getBackend() !== 'webgpu') throw new Error('WebGPU backend unavailable');
      return;
    } catch {
...
async function ensureBackend(backend?: KataGoBackendPreference): Promise<void> {
  ...
  loadedModelUrl = null;
  loadedModelCacheStatus = 'unavailable';
  search = null;
  searchKey = null;
...
function installModel(
  nextModel: KataGoModelV8Tf,
  parsed: ParsedKataGoModelV8,
  modelUrl: string,
  ...
  loadedModelCacheStatus = cacheStatus;
  search = null;
  searchKey = null;
}
...
function postModelProgress(
  ...
  post({ type: 'katago:model_progress', modelUrl, stage, loadedBytes, totalBytes });
}

async function readModelResponse(
  response: Response,
  modelUrl: string,
  stage: 'reading-cache' | 'downloading',
): Promise<Uint8Array> {
  const headerTotal = Number(response.headers.get('content-length'));
  const totalBytes = Number.isFinite(headerTotal) && headerTotal > 0 ? headerTotal : 0;
  if (!response.body) {
    const data = new Uint8Array(await response.arrayBuffer());
    postModelProgress(modelUrl, stage, data.byteLength, totalBytes || data.byteLength);
    return data;
  }
  ...
  while (true) {
    ...
    if (now - lastReportedAt >= 100 || (totalBytes > 0 && loadedBytes >= totalBytes)) {
      lastReportedAt = now;
      postModelProgress(modelUrl, stage, loadedBytes, totalBytes);
    }
  }
  ...
  postModelProgress(modelUrl, stage, loadedBytes, totalBytes || loadedBytes);
  return data;
}

async function fetchModelBytes(modelUrl: string): Promise<{
  bytes: Uint8Array;
  cacheStatus: 'hit' | 'stored' | 'unavailable';
}> {
  postModelProgress(modelUrl, 'checking-cache', 0, 0);
  let cache: Cache | null = null;
  try {
    if (typeof caches !== 'undefined') cache = await caches.open('ac-game-go-models-v1');
    const cached = await cache?.match(modelUrl);
    if (cached) {
      return { bytes: await readModelResponse(cached, modelUrl, 'reading-cache'), cacheStatus: 'hit' };
    }
  } catch {
    cache = null;
  }
  ...
  const cacheWrite = cache
    ? cache.put(modelUrl, response.clone()).then(() => true).catch(() => false)
    : Promise.resolve(false);
  const bytes = await readModelResponse(response, modelUrl, 'downloading');
  postModelProgress(modelUrl, 'storing-cache', bytes.byteLength, bytes.byteLength);
  const stored = await cacheWrite;
  return { bytes, cacheStatus: stored ? 'stored' : 'unavailable' };
}
...
async function ensureModel(modelUrl: string, backend?: KataGoBackendPreference): Promise<void> {
  ...
  const loaded = await fetchModelBytes(modelUrl);
  postModelProgress(modelUrl, 'parsing', loaded.bytes.byteLength, loaded.bytes.byteLength);
  const data = maybeUngzip(loaded.bytes);

  const parsed = parseKataGoModelV8(data);
  const attemptedFallbacks = new Set<KataGoBackendPreference>();
  while (true) {
    try {
      postModelProgress(modelUrl, 'warming-up', loaded.bytes.byteLength, loaded.bytes.byteLength);
      installModel(await createWarmedModel(parsed), parsed, modelUrl, loaded.cacheStatus);
      return;
    } catch (err) {
...
async function handleMessage(msg: KataGoWorkerRequest): Promise<void> {
  if (msg.type === 'katago:init') {
    ...
    post({
      ...
      cacheStatus: loadedModelCacheStatus,
    });
    return;
  }
